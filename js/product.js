...
function toggleWishlist(){var b=document.getElementById('wishlist-btn');if(b)b.classList.toggle('wishlisted');if(!product){showToast('Product still loading, please wait...');b.classList.toggle('wishlisted');return}var w=JSON.parse(localStorage.getItem('bd_wishlist')||'[]');var idx=w.findIndex(function(x){return x.id===product.id});if(idx>=0){w.splice(idx,1);showToast('Removed from wishlist')}else{w.push({id:product.id,title:product.title,price:product.price,image:product.image||(product.images||[])[0]||'',category:product.category,added:new Date().toISOString()});showToast('Added to wishlist ♡')}localStorage.setItem('bd_wishlist',JSON.stringify(w));}

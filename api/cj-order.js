export default async function handler(req, res) {
  ...
  async function getToken() {
    const r = await fetch('https://developers.cjdropshipping.com/api2.0/v1/authentication/getAccessToken', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ apiKey: API }) });
  ...
  try {
    ...
    const r = await fetch('https://developers.cjdropshipping.com/api2.0/v1/shopping/order/createOrderV2', { method: 'POST', headers: {'CJ-Access-Token': t, 'Content-Type':'application/json'}, body: JSON.stringify(body) });
    const dd = await r.json();
    if (dd.code === 200 && dd.result) {
      res.status(200).json({ success: true, c6_order_id: dd.data.orderId, order_number: oid, message: 'Order synced to CJ Fulfillment' });

// Processes Stripe token from Google Pay PAYMENT_GATEWAY server-side.
// Google Pay creates the Stripe token → we process with SECRET key (no pub key needed here).
export default async function handler(req, res) {
  ...
  try {
    // Step 1: Create PaymentMethod from the Stripe token
    ...
    if (pmData.error) throw new Error('PaymentMethod: ' + pmData.error.message);
